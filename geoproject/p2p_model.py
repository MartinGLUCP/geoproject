...
Deprecated version of tensorflow 
...
"""
from tensorflow.keras.layers import layers.Conv2D
from tensorflow.keras.layers import layers.Conv2DTranspose
from tensorflow.keras.layers import layers.LeakyReLU
from tensorflow.keras.layers import layers.Activation
from tensorflow.keras.layers import layers.Concatenate
...
#Given an input image, the Discriminator outputs the likelihood of the image being real.
    #Binary classification - true or false (1 or 0). So using sigmoid layers.Activation.
...
def define_discriminator(image_shape):
	...
	init = tf.keras.initializers.RandomNormal(stddev=0.02) #As described in the original paper
    
	# source image input
	in_src_image = keras.Input(shape=image_shape)  #Image we want to convert to another image
	# target image input
	in_target_image = keras.Input(shape=image_shape)  #Image we want to generate after training. 
    
	# layers.Concatenate images, channel-wise
	...
	# define model
	model = keras.Model([in_src_image, in_target_image], patch_out)
	# compile model
...
# keras.utils.plot_model(disc_model, to_file='disc_model.png', show_shapes=True)
...
def define_generator(image_shape=(256,256,3)):
	...
	# output
	g = layers.Conv2DTranspose(image_shape[2], (4,4), strides=(2,2), padding='same', kernel_initializer=init)(d7) #Modified 
	out_image = layers.Activation('tanh')(g)  #Generates images in the range -1 to 1. So change inputs also to -1 to 1
	# define model
	model = keras.Model(in_image, out_image)
	return model

# gen_model = define_generator((256,256,3))
# keras.utils.plot_model(gen_model, to_file='gen_model.png', show_shapes=True)
...
def train(d_model, g_model, gan_model, dataset, n_epochs=100, n_batch=1):
	...
	# unpack dataset
	trainA, trainB = dataset
	# calculate the number of batches per training epochs
